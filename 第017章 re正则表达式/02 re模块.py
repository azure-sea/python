...
# ret = re.findall('\d+','sj氨基酸的123.123l1e1') # 正则表达式, 带匹配的字符串, flag
# print(ret)

# # search : 只匹配从左到右的第一个,得到的不是直接的结果，而是一个变量,通过这个变量的group方式来获取结果
# #          如果没有匹配到,会返回None, 使用group会报错
# ret = re.search('\d+','sj氨基酸的123.123l1e1')
# print(ret)   # 内存地址,这是一个正则匹配的结果
# print(ret.group()) # 通过ret.group() 获取真正的结果


# ret = re.search('\d','asdkjdvafqdvq')
# if ret:
#     print(ret)
# # 字符串处理的扩展：替换 切割
# # split
# s = 'asjc123vo2sdv4cvhqd'
# ret = re.split('\d+',s)
# print(ret)

# sub # 谁  老的  新的  替换次数
# ret = re.sub('\d+','H','jgagsivHw1fnwh12')
# print(ret)
# ret = re.sub('\d+','H','jgagsivHw1fnwh1',1)
# print(ret)

# # subn  返回一个元组，第二个元素是替换的次数。
# ret = re.subn('\d+','H','jgagsivHw1fnwh12')
# print(ret)
